...
from application.utils.exceptions import DatabaseError, InvalidTemplateException, InvalidClassificationType
...
def with_db_session(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        except InvalidClassificationType as exception:
            log.info("Rolling-back database session.")
            session.rollback()
            raise InvalidClassificationType(exception.error, exception.status_code)
